...
class refund_returnModel extends Model{
    ...
    public function editOrderRefund($refund) {
        ...
        $goods_id = intval($refund['goods_id']);
        ...
        if ($refund_id > 0) {
            ...
            $goods_store_name = $goods_info['store_name'];
            //echo "goods_class_name:".$goods_class_name;
...
//                define('SHOPNC_ROOT', substr(dirname(__FILE__), 0, -5));
//                $data = require(SHOPNC_ROOT.'/cache/setting.php');
//                require_once SHOPNC_ROOT.'/config/config.ini.php';
//                define('BASE_DATA_PATH', str_replace('\\', '/', dirname(dirname(__FILE__))));
//                echo "BASE_DATA_PATH:".BASE_DATA_PATH;
//                require_once(BASE_DATA_PATH.'/config/config.ini.php');
//                echo "txgoods_class_name:".$config['txgoods_class_name'];
                if ($goods_store_name != TX_STORE_NAME && $predeposit_amount > 0) {//退预存款
                ...
                //提现送积分
                if($goods_store_name == TX_STORE_NAME){
                    $points_model = Model('points');
                    $points_model->savePointsLog('other',array('pl_memberid'=>$order['buyer_id'],'pl_membername'=>$log_array['member_name'],'pl_points'=>intval($predeposit_amount/100),'pl_desc'=>('提现赠送积分，退货号：'.$refund_id)),true);
                }
